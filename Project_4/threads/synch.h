...
class Condition {
  ...
  private:
	...
	Lock *WaitingLock; // This is to keep track of which lock are we currently waiting for. Eg - Clerk1 or Clerk2 or whatever
	List *ConditionWaitQueue;
	
};

//class that saves the Response messages that need to be sent to the client
class Response
{
	public:
		int networkID;
		...
		Response(int netID, int mBoxID, bool isError, char* msg);
};

// ServerLock implements the abstraction of actual locks
class ServerLock
{
	public: 
		void Acquire(int ownerNetworkID, int ownerMailBoxID);
		
		// releases the lock and returns the address of the client to which the message is to be sent
		Response* Release(int ownerNetworkID, int ownerMailBoxID); 
		...
		~ServerLock();
		char* getName(){return name;}
		List* getLockList(){return LockWaitQueue;}
		
		int refCount;
		bool deleteCalled;

		int getOwnerNetworkID();
		int getOwnerMailBoxID();
	
	private :
		LockStatus lockState;
		List *LockWaitQueue; // the waiting queue of the lock
		char *name; // for debugging purposes
		int ownerNetworkID;
		int ownerMailboxID;
};

// ServerCondition implements the abstraction of actual condition variables
class ServerCondition
{
	private:
		char *WaitingLock;
	...
	public:
		...
		char* getName(){return name;}
		// make the client thread wait on the condition lock
		void Wait(char* WaitLock, int networkID, int mailboxID);
		// Signal any other client thread waiting on this condition lock
		Response* Signal(char* WaitLock);
		// Signal all  the clients waiting on the condition lock
		void Broadcast(char* WaitLock);
};
