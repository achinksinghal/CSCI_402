...
{
	if(networkID == ownerNetworkID && mailboxID == ownerMailboxID)
	{
		return;
	}
...
//
//---------------------------------------------------------------------
void ServerCondition :: Wait(char *waitLock, int networkID, int mailboxID)
{	
	if(!strcmp(waitLock,"")) // the lock passed here hasn't been created yet
	{
		Response *response = new Response(networkID, mailboxID, true, "Null Lock");
		printf("The lock doesn't match with cv\n");
		return;
	}
	
	if(!strcmp(WaitingLock,"")) // This is the first client to invoke this condition variable
	{
		strcpy(WaitingLock,waitLock);
	}
	
	if(strcmp(waitLock,WaitingLock) && !strcmp(WaitingLock,"")) // this is not the same lock as everyone else is waiting on
	{
		Response *response = new Response(networkID, mailboxID, true, "BadLock");
		printf("The lock doesn't match with cv\n");
		return;
	}
...
//
//---------------------------------------------------------------------
Response* ServerCondition :: Signal(char *waitLock)
{
	if(strcmp(WaitingLock,waitLock)) // the locks don't match
	{
		return false;
	}
	else
...
//
//---------------------------------------------------------------------
/*void ServerCondition :: Broadcast(int waitLock)
{
