...
void Exec_Thread(int inner)
{
	...
	currentThread -> space -> RestoreState();
	//printf("\nACHINTYA in Exec_thread %d process exec\n", inner);//invalidateTLB();
...
SpaceId Exec_Syscall(unsigned int vaddr, int length)
{
	...
	//printf("\nACHINTYA constructor process %d called\n", nextProcessID);fflush(stdout);//invalidateTLB();

	if(nextProcessID == -1)
	{
		return -1;
	}
	//printf("\nACHINTYA return %d called\n", nextProcessID);//invalidateTLB();
...
int SendToServer(char *msg)
{
	...
	outPktHdr.to = 0; // we assume that the network id of the server is 0
	...
	if(!strcmp(requestType,"LOCK_CREATE")) // the request was for lock creation
	{
		...
		{
			printf("The lock number assigned is %d\n", lockID);
			return lockID;
		}
	}
	...
	else if(!strcmp(requestType,"LOCK_REL"))
	{
		...
		{
			cout << "\n Server threw an exception "  << responseMsg;
			return -1;
		}
	...
	else if(!strcmp(requestType,"CV_CREATE"))
	{
		...
		{
			printf("The condition variable assigned is %d\n", cvID);
			return cvID;
		}
	}
...
void AcquireLock_Syscall(int lockID)
{
	...
	msgStream << "LOCK_ACQ " << lockID << endl;
	msg = const_cast<char *>(msgStream.str().c_str());
	
	int status = SendToServer(msg);
	if(status != -1)
	printf("\n Lock %d acquired by the client\n", lockID);
	//delete msg;
...
void ReleaseLock_Syscall(int lockID)
{
	...
	msgStream << "LOCK_REL " << lockID << endl;
	msg = const_cast<char *>(msgStream.str().c_str());
...
void DestroyLock_Syscall(int lockID)
{
	...
	msgStream << "LOCK_DES " << lockID << endl;
	msg = const_cast<char *>(msgStream.str().c_str());
...
int CreateCV_Syscall(unsigned int vaddr)
{
	...
	msgStream << "CV_CREATE " << cvName << endl;
	msg = const_cast<char *>(msgStream.str().c_str());
	
	int cvID = SendToServer(msg);
	delete msg;
	return cvID;
}
...
void WaitCV_Syscall(int lockID, int cvID)
{
	...
	msgStream << "CV_WT " << cvID << " " << lockID <<endl;
	msg = const_cast<char *>(msgStream.str().c_str());
...
void SignalCV_Syscall(int lockID, int cvID)
{
	...
	msgStream << "CV_SIG " << cvID << " " << lockID <<endl;
	msg = const_cast<char *>(msgStream.str().c_str());
...
void BroadcastCV_Syscall(int lockID, int cvID)
{
	...
	msgStream << "CV_BCST " << cvID << " " << lockID <<endl;
	msg = const_cast<char *>(msgStream.str().c_str());
...
int GetMV(int mvID)
{
	...
	msgStream << "MV_GET " << mvID << endl;
	msg = const_cast<char *>(msgStream.str().c_str());
	
	int retVal = SendToServer(msg);
	delete msg;
	return retVal;
}
...
int SetMV(int mvID, int val)
{
	...
	msgStream << "MV_SET " << mvID << " " << val << endl;
	msg = const_cast<char *>(msgStream.str().c_str());
...
void ExceptionHandler(ExceptionType which) {
    ...
    {
//	    cout<<"PageFaultException occured:"<<endl;
	    {
		int currentVPN = machine->ReadRegister(BadVAddrReg)/PageSize;
		//currentThread->space->populateTLB(currentVPN);
		    //printf("\nACHINTYA Process=%d TLB miss for virtualPage=%d BadVAddrReg=%d PageSize=%d", currentThread->GetParentProcess(), currentVPN, machine->ReadRegister(BadVAddrReg),PageSize);

		if(populateTLBFromIPT(currentVPN) == 0)
		{
			currentThread->space->populateTLBFromPagetable(currentVPN);
		}
	    }
    }
    else 
